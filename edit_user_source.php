...
//                $result = mysqli_query($con, "UPDATE `user` SET `password` = MD5('" . $_POST['password'] . "'), `status` = " . $_POST['status'] . ", `last_updated`=" . time() . " WHERE `user`.`id` = " . $_POST['user_id'] . ";");
                $result = mysqli_query($conn, "update tbl_users set password = '$password', status = '$status', last_updated=".time()."  where id = '$user_id'");
                if (!$result) {
            ...
            <?php } else { ?>
                <div id="edit-notify" class="box-content">
                    ...
                    <a href="./edit_user_source.php?id=<?= $_POST['user_id'] ?>">Quay lại sửa tài khoản</a>
                </div>
            <?php
            }
        } else {
            $result = mysqli_query($conn, "SELECT * FROM tbl_users where `id`=" . $_GET['id']);
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    ...
                    <form action="./edit_user_source.php?action=edit" method="Post" autocomplete="off">
